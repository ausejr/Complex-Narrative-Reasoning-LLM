...
def structured_reasoning(info, answer_info, answer, step,n):
    ...
    if step == 1:
        ...
        **3. 全面探索疑点与真凶锁定：**
            ...
            * 3.2 **真凶锁定**：持续进行探索，直到你确信所有相关信息都已被充分挖掘，并且你已经锁定了真凶。**
        """
        prompt_tail2 = f"""
        ------------------------
    ...
    if step == 0 and n==2: prompt=f"""你提出的问题以及答案{answer},你不能继续探索了，请输出答案"""
    elif step==0:  prompt = f""" 你提出的问题以及答案{answer},继续探索"""
    else:  prompt = prompt_head + prompt_body + prompt_tail1 + prompt_tail2
